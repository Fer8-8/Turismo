...
import { useGetNearbyNaturePlaces } from "../../api/get-nearby-nature-places";
import { NearbyCulturalPlacesItem } from "../culture/nearby-cultural-places-item";

const PLACES_LIMIT = 5;

type NearNaturePlacesListProps = {
  userLocation: { latitude: number; longitude: number };
};

export function NearNaturePlacesList({
  userLocation,
}: NearNaturePlacesListProps) {
  const { latitude, longitude } = userLocation;
  const { data } = useGetNearbyNaturePlaces(latitude, longitude, PLACES_LIMIT);
