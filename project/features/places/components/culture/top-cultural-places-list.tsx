...
import TopCulturalPlacesitem from "./top-cultural-places-item";

const PLACES_LIMIT = 10;

export function TopCulturalPlacesList() {
  const { data: categories } = useGetCategories();
  const cultureCategory = categories?.find((cat) => cat.category === "Cultura");

  const { data } = useGetAllPlacesByCategory(
    cultureCategory?.id_category,
    PLACES_LIMIT
  );
  ...
  function handleSeeAll() {
    if (cultureCategory) {
      router.push({
        ...
        params: { categoryId: cultureCategory.id_category },
      });
    }
  }
